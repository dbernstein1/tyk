...
func keyLoader(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		if strings.Contains(r.URL.Path, "/key/refresh") && appName != "" {
			obj, code = refreshKeys(ADD, appName)
		} else {
			obj, code = updateKeys(r, ADD)
		}
	case "DELETE":
		if keyID != "" && appName != "" {
...
func updateKeys(r *http.Request, e Event) (interface{}, int) {
	//Add key to Redis and call addOrDeleteJWTKey
	...
	err = addOrDeleteJWTKey(e, jwtDef.AppName)
	if err != nil {
...
func refreshKeys(e Event, appName string) (interface{}, int) {
...
func addOrDeleteJWTKey(e Event, appName string) error {
	...
	for _, api := range apis {
		...
		if !strings.HasPrefix(api, JWTKeyPrefix) {
			...
			if jsonApi["app_name"] == appName && jsonApi["enable_jwt"] == true {
	...
	//Get all existing JWT Keys for appName
	keys, err := redis.Strings(c.Do("KEYS", JWTKeyPrefix+appName+"-"+"*"))
	if err != nil {
	...
	log.Debug(fmt.Sprintf("Key List %v ", keys))
	...
	for _, key := range keys {
		...
		log.Debug(fmt.Sprintf("JWT Public Key %s", jwtKey))
		count := 0
		for {
...
func deleteAPIById(apiID string) (interface{}, int) {
	...
	//TODO - Read appName from apiData and pass it to addOrDeleteJWTKey
...
func deleteAPIByService(service string) (interface{}, int) {
	...
	//remove api id from all JWT keys
	//TODO - Read appName from api sepc and pass it to addOrDeleteJWTKey
	apiDef := &apidef.APIDefinition{}
	if err := json.NewDecoder(strings.NewReader(apiData)).Decode(apiDef); err != nil {
		return apiError("Could not decode api definition"), http.StatusInternalServerError
	}
