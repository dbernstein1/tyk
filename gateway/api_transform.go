...
func (gw *Gateway) updateKeys(e Event) (interface{}, int) {
	err := gw.addOrDeleteJWTKey(ADD)
	if err != nil {
...
func (gw *Gateway) addOrUpdateApi(r *http.Request) (interface{}, int) {
	...
	platform, err := gw.getPlatform(SystemConfigFilePath)
	if err != nil {
...
func (gw *Gateway) addOrDeleteJWTKey(e Event) error {
	...
	for _, jwtMeta := range jwtDefinitions.JWTDefinitions {
		...
		for {
			...
			ret := gw.processJWTApiKey(tykConf, JWTAPIMap, jwtMeta.JWTPublicKeyPath, jwtMeta.JWTAPIKeyPath, "localhost", e)
			count++
			if ret == true {
				break
			} else if count < 3 {
...
func (gw *Gateway) deleteAPIById(apiID string) (interface{}, int) {
	...
	err = gw.addOrDeleteJWTKey(DELETE)
	if err != nil {
...
func (gw *Gateway) getPlatform(SysConfPath string) (string, error) {
	var ret = "aci"
