...
func addOrDeleteJWTKey(e Event, appName string) error {
	...
	//Get all KEYs belonging to appName
	...
	log.Debug(fmt.Sprintf("addOrDeleteJWTKey All Key List %v ", keys))
	...
	for _, key := range keys {
		...
		if Contains(jwtDef.AppNameList, appName) {
			...
			log.Debug(fmt.Sprintf("Skip updating JWT key %s", jwtDef.JWTAPIKey))
		}
