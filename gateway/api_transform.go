...
func (gw *Gateway) handleGetKey(appName string, kid string) (interface{}, int) {
	c := GetRedisConn()
	defer c.Close()
...
func (gw *Gateway) deleteJWTKey(keyID string, appName string) (interface{}, int) {
	...
	// JWTApiKey = keyID
	endPoint := gw.getTykEndpoint("localhost", TykJWTAPIKeyEndpoint) + keyID
...
//On Posting JWT Key
func (gw *Gateway) updateKeys(r *http.Request) (interface{}, int) {
	//Add key to Redis and call addOrUpdateJWTKey
	var jwtDef JWTDefinition
	//Receive JWT payload
	data, err := gw.receivePayload(r)
	if err != nil {
	...
	err = gw.addOrUpdateJWTKey(jwtDef)
	if err != nil {
...
func (gw *Gateway) refreshKeys(appName string) (interface{}, int) {
	c := GetRedisConn()
	defer c.Close()
...
func (gw *Gateway) receivePayload(r *http.Request) ([]byte, error) {
	...
	if gw.GetConfig().DynamicAPIConnTimeout == 0 {
		connTimeout = DynamicAPIConnTimeout
	} else {
		connTimeout = gw.GetConfig().DynamicAPIConnTimeout
	}
		...
		for _, api := range apis {
			...
			if api.EnablePythonMiddleware {
				...
				if _, err := os.Stat(middlewareBundlePath); os.IsNotExist(err) {
					...
					_, mErr := gw.copyFile(middlewareSource, middlewareDestination)
					if mErr != nil {
						return apiError("Middleware Error"), http.StatusInternalServerError
					}
					...
					_, maErr := gw.copyFile(manifestSource, manifestDestination)
					if maErr != nil {
						return apiError("Middleware Error"), http.StatusInternalServerError
					}
...
func (gw *Gateway) addOrUpdateJWTKey(jwtDef JWTDefinition) error {
...
func (gw *Gateway) processJWTApiKey(tykConf map[string]interface{},
	...
	var endPoint = gw.getTykEndpoint(host, TykJWTAPIKeyEndpoint) + JWTKey
	...
	if (e == ADD && len(JWTAPIMap) > 0) || (e == DELETE && len(JWTAPIMap) > 0) {
		...
		if resp.StatusCode == 200 {
			...
			ret := gw.checkIfJwtKeyCreated(tykConf, JWTKey, host)
			if !ret {
				return ret
			}
		} else {
...
func (gw *Gateway) checkIfJwtKeyCreated(tykConf map[string]interface{}, jwtKey string, host string) bool {
...
func (gw *Gateway) deleteAPIById(apiID string) (interface{}, int) {
	...
	err = gw.addOrDeleteJWTKey(DELETE, apiDef.AppName)
	if err != nil {
...
func (gw *Gateway) getInbandIP(SysConfPath string) (string, error) {
...
func (gw *Gateway) copyFile(src, dst string) (int64, error) {
...
func (gw *Gateway) getTykEndpoint(host string, path string) string {
	url := url.URL{
		Scheme: "https",
		Host:   host + ":" + strconv.Itoa(gw.GetConfig().ControlAPIPort),
		Path:   path,
	}
	return url.String()
}